...
Trang web chấm điểm KPI. **Backend + nghiệp vụ nằm ở project `../KPI`** (Apps Script,
sheet `CẤU HÌNH HỆ THỐNG` + 4 sheet chấm). Đọc `../KPI/AGENTS.md` và
`../KPI/docs/kien-truc-va-canh-bao.md` trước khi đổi contract API.

## Ràng buộc

- FE tĩnh, **không framework** — giữ nhẹ. Không thêm dependency nếu không thật cần.
- Contract API = `src/types.ts` phải khớp `../KPI/appscript/KPI_CHAM_TO.gs` (`doPost` router).
  Sửa 1 bên thì sửa bên kia.
